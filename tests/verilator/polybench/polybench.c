...
int
polybench_papi_start_counter(int evid)
{
...
void
polybench_papi_stop_counter(int evid)
{
#ifdef _OPENMP
#pragma omp parallel
  {
    if (omp_get_thread_num() == polybench_papi_counters_threadid)
    {
      ...
      if ((retval = PAPI_remove_event(polybench_papi_eventset, polybench_papi_eventlist[evid]))
          != PAPI_OK)
...
polybench_timer_print()
{
  ...
  if (polybench_program_total_flops == 0)
  {
    ...
    printf(
        "%0.2lf\n",
        (polybench_program_total_flops / (double)(polybench_t_end - polybench_t_start))
            / 1000000000);
#else
#ifndef POLYBENCH_CYCLE_ACCURATE_TIMER
  printf("%0.6f\n", polybench_t_end - polybench_t_start);
...
grow_alloc_table()
{
      ...
      || _polybench_alloc_table->nb_avail_entries != 0)
  {
    ...
    fprintf(
        stderr,
        "[ERROR] Inter-array padding requires to use polybench_alloc_data and "
        "polybench_free_data\n");
    exit(1);
  }
...
static void *
register_padded_pointer(void * ptr, size_t orig_sz, size_t padded_sz)
{
  if (_polybench_alloc_table == NULL)
  {
    fprintf(
