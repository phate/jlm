...
DeltaTest2::SetupRvsdg()
{
  ...
  auto SetupF2 = [&](lambda::output * f1, delta::output * d1, delta::output * d2)
  {
    ...
    st = StoreNode::Create(cvd2, b42, { call.GetMemoryStateOutput() }, 4);
...
ImportTest::SetupRvsdg()
{
  ...
  auto SetupF2 = [&](lambda::output * f1, jlm::rvsdg::output * d1, jlm::rvsdg::output * d2)
  {
    ...
    auto st = StoreNode::Create(cvd1, b2, { memoryStateArgument }, 4);
    auto & call = CallNode::CreateNode(cvf1, f1->node()->type(), { iOStateArgument, st[0] });
    st = StoreNode::Create(cvd2, b21, { call.GetMemoryStateOutput() }, 4);
...
EscapedMemoryTest2::SetupRvsdg()
{
  ...
  auto SetupReturnAddressFunction = [&]()
  {
    ...
    FunctionType functionType(
        { &iOStateType, &memoryStateType },
        { &p8, &iOStateType, &memoryStateType });
...
MemcpyTest::SetupRvsdg()
{
      ...
      [&](delta::output & localArray, delta::output & globalArray, lambda::output & lambdaF)
  {
    ...
    auto memcpyResults =
        MemCpyOperation::create(bcGlobalArray, bcLocalArray, twenty, zero, { memoryStateArgument });
...
MemcpyTest2::SetupRvsdg()
{
  ...
  auto SetupFunctionG = [&]()
  {
    ...
    auto memcpyResults = MemCpyOperation::create(ldS2[0], ldS1[0], c128, cFalse, { ldS1[1] });
...
MemcpyTest3::SetupRvsdg()
{
  ...
  auto memcpyResults =
      MemCpyOperation::create(allocaResults[0], pArgument, eight, cFalse, { memoryState });
  ...
  memcpyResults = MemCpyOperation::create(ld[0], gep2, three, cFalse, { ld[1] });
...
VariadicFunctionTest1::SetupRvsdg()
{
  ...
  FunctionType lambdaFType(
      { &pointerType, &iOStateType, &memoryStateType },
      { &iOStateType, &memoryStateType });
  FunctionType lambdaGType({ &iOStateType, &memoryStateType }, { &iOStateType, &memoryStateType });
  ...
  // Setup f()
  {
    ...
    auto storeResults =
        StoreNode::Create(CallH_->Result(0), three, { CallH_->GetMemoryStateOutput() }, 4);
