...
TestLoad1()
{
	auto validate_ptg = [](const jlm::aa::PointsToGraph & ptg, const LoadTest1 & test)
	{
		...
		assertTargets(pload_p, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});

		assertTargets(plambda, {&lambda});
		assertTargets(lambarg0, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
	};
...
TestLoadFromUndef()
{
  ...
  {
    assert(pointsToGraph.NumAllocatorNodes() == 1);
...
TestGetElementPtr()
{
	auto validatePtg = [](const jlm::aa::PointsToGraph & ptg, const GetElementPtrTest & test)
	{
		...
		*/
		...
		assertTargets(gepX, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
		assertTargets(gepY, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
	};
...
TestBitCast()
{
	auto validatePtg = [](const jlm::aa::PointsToGraph & ptg, const BitCastTest & test)
	{
		...
		assertTargets(lambdaArg, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
		assertTargets(bitCast, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
	};
...
TestGamma()
{
	auto validate_ptg = [](const jlm::aa::PointsToGraph & ptg, const GammaTest & test)
	{
		...
		for (size_t n = 1; n < 5; n++) {
			...
			assertTargets(lambda_arg, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
		}
		...
		for (size_t n = 0; n < 4; n++) {
			...
			assertTargets(argument0, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
			assertTargets(argument1, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
		}

		for (size_t n = 0; n < 4; n++) {
			auto & output = ptg.GetRegisterNode(*test.gamma->exitvar(0));
			assertTargets(output, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
		}
	};
...
TestTheta()
{
	auto validate_ptg = [](const jlm::aa::PointsToGraph & ptg, const ThetaTest & test)
	{
		...
		assertTargets(lambda_arg1, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
		assertTargets(lambda_out, {&lambda});

		assertTargets(gep_out, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});

		assertTargets(theta_lv2_arg, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
		assertTargets(theta_lv2_out, {&ptg.GetUnknownMemoryNode(), &ptg.GetExternalMemoryNode()});
	};
...
TestExternalMemory()
{
  auto ValidatePointsToGraph = [](const jlm::aa::PointsToGraph & pointsToGraph, const ExternalMemoryTest & test)
  {
    ...
    assertTargets(lambdaFArgument0, {&pointsToGraph.GetUnknownMemoryNode(), &pointsToGraph.GetExternalMemoryNode()});
    assertTargets(lambdaFArgument1, {&pointsToGraph.GetUnknownMemoryNode(), &pointsToGraph.GetExternalMemoryNode()});
  };
