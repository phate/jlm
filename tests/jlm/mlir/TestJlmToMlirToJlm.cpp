...
TestAlloca()
{
  ...
  nf->set_mutable(false);
  {
...

    {
      ...
      {
        if (auto allocaOp = dynamic_cast<const alloca_op *>(&node.operation()))
        {
...
TestLoad()
{
  ...
  nf->set_mutable(false);
  {
    ...
    auto pointerArgument = lambda->fctargument(2);
...

    {
      ...
      assert(region->nnodes() == 1);
      auto convertedLambda = jlm::util::AssertedCast<lambda::node>(region->nodes.first());
      assert(is<lambda::operation>(convertedLambda));

      assert(convertedLambda->subregion()->nnodes() == 1);
      assert(is<LoadNonVolatileOperation>(convertedLambda->subregion()->nodes.first()));
      auto convertedLoad =
          dynamic_cast<const LoadNonVolatileNode *>(convertedLambda->subregion()->nodes.first());
...
TestStore()
{
  ...
  nf->set_mutable(false);
  {
    ...
    auto memoryStateArgument = lambda->fctargument(1);
    auto pointerArgument = lambda->fctargument(2);
    auto bitsArgument = lambda->fctargument(3);
...

    {
      ...
      assert(convertedLambda->subregion()->nnodes() == 1);
      assert(is<StoreNonVolatileOperation>(convertedLambda->subregion()->nodes.first()));
      auto convertedStore =
          dynamic_cast<const StoreNonVolatileNode *>(convertedLambda->subregion()->nodes.first());
