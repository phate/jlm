...
alloca_conv(rvsdg::Region * region)
{
  for (auto & node : rvsdg::TopDownTraverser(region))
  {
    ...
    {
      if (auto po = dynamic_cast<const jlm::llvm::AllocaOperation *>(&(simplenode->GetOperation())))
      {
        ...
        auto req_outs =
            LocalMemoryRequestOperation::create(*mem_outs[1], load_addrs, store_operands);
        ...
        if (merge_node
            && dynamic_cast<const llvm::MemoryStateMergeOperation *>(&merge_node->GetOperation()))
        {
