...
void distribute_constant(const rvsdg::simple_op &op, rvsdg::simple_output* out){
    ...
    while(changed) {
        ...
        for (auto user: *out) {
            if (auto ti = dynamic_cast<rvsdg::theta_input *>(user)) {
                ...
                if (res->origin() == arg) {
                    // pass-through
                    auto arg_replacement = dynamic_cast<rvsdg::simple_output *>(rvsdg::simple_node::create_normalized(
                            ti->node()->subregion(), op, {})[0]);
                    ti->argument()->divert_users(arg_replacement);
                    ti->output()->divert_users(rvsdg::simple_node::create_normalized(out->region(), op, {})[0]);
...
hls::distribute_constants(rvsdg::region *region) {
    //push constants down as far as possible, since this is cheaper than having forks and potentially buffers for them
