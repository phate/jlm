    ...
    const std::shared_ptr<const jlm::rvsdg::FunctionType> & allocaFunctionType)
{
  ...
  for (auto & node : rvsdg::TopDownTraverser(region))
  {
    ...
    {
      const auto & op = simplenode->GetOperation();
      if (auto loadOp = dynamic_cast<const jlm::llvm::LoadNonVolatileOperation *>(&op))
      {
      ...
      else if (auto ao = dynamic_cast<const jlm::llvm::AllocaOperation *>(&op))
      {
        ...
        auto constant_operation =
            dynamic_cast<const llvm::IntegerConstantOperation *>(&constant_node->GetOperation());
      ...
      else if (auto so = dynamic_cast<const jlm::llvm::StoreNonVolatileOperation *>(&op))
      {
        auto addr = node->input(0)->origin();
        JLM_ASSERT(rvsdg::is<jlm::llvm::PointerType>(addr->Type()));
