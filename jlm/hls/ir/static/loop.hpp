...
public:
   ...
   */
  void
  print_nodes_registers() const;

  // TODO doc
  void
  print_fsm() const;

  inline const jlm::rvsdg::substitution_map *
  get_reg_smap() const
  {
    return &reg_smap_;
  }
   ...
   * This method adds a node to the loop_node either by adding an operation to the compute region or
   * by using an already implemented one. Also adds a register in the control region for each of its
   * outputs. This will also add inputs to the muxes in the control region to route the node node.
   ...
   * This goes through every node in the compute region to check if the operation is already
   * implemented. \param op The operation to check. \return The node that implements the operation
   * if it is already implemented, nullptr otherwise.
   */
  jlm::rvsdg::node *
...
private:
  ...
  // TODO rename
  jlm::rvsdg::substitution_map reg_smap_; // Maps the original node ouput with its substitute
                                          // register output, also maps arguments
};
