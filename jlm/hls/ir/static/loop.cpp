...
loop_node::get_mux(jlm::rvsdg::node_input * node) const
{
  ...
  if (!node_output)
    JLM_UNREACHABLE("SHLS: call of function loop_node::get_mux() with invalid argument");
...
{
  auto input = jlm::rvsdg::structural_input::create(
      this,
      theta_input->origin(),
      theta_input->origin()->Type());
  auto output = jlm::rvsdg::structural_output::create(this, theta_input->origin()->Type());

  auto argument_in =
      jlm::rvsdg::argument::create(control_subregion(), input, theta_input->origin()->Type());
...
loop_node::add_node(jlm::rvsdg::node * node)
{
  ...
  if (auto implemented_node = is_op_implemented(node->operation()))
  {
    ...
    std::cout << "Node operation " << node->operation().debug_string() << " already implemented"
              << std::endl;
    ...
    for (size_t i = 0; i < node->ninputs(); i++)
    {
      ...
      if (!mux)
        JLM_UNREACHABLE("SHLS loop_node::add_node() : mux not found");

      auto input_new_origin = reg_smap_.lookup(node->input(i)->origin());
      if (!input_new_origin)
        JLM_UNREACHABLE("SHLS loop_node::add_node() : node input origin not found in reg_smap_");

      mux = mux_add_input(mux, input_new_origin);

      auto mux_op = dynamic_cast<const jlm::static_hls::mux_op *>(&mux->operation());
      if (!mux_op)
        JLM_UNREACHABLE("Mux operation not found");
    ...
    for (size_t i = 0; i < node->ninputs(); i++)
    {
      ...
      if (!input_new_origin)
        JLM_UNREACHABLE(util::strfmt(
                            "SHLS loop_node::add_node() : node input origin not found in reg_smap_ "
                            "for node with op ",
  ...
  for (size_t i = 0; i < node->noutputs(); i++)
  {
    ...
    auto reg = reg_op::create(
        *reg_store_origin,
        *backedge_result->argument(),
        jlm::util::strfmt(compute_node->operation().debug_string(), ":", i));
...
loop_node::add_loopback_arg(jlm::rvsdg::theta_input * theta_input)
{
  ...
  if (!new_arg_out)
    JLM_UNREACHABLE("SHLS: loop_node::add_loopback_arg() cannot find the argument in reg_smap_");

  auto new_arg = dynamic_cast<jlm::rvsdg::argument *>(new_arg_out);
  if (!new_arg)
    JLM_UNREACHABLE("SHLS: loop_node::add_loopback_arg() cannot cast to arg");
  ...
  for (auto user : old_users)
  {
    ...
    if (!mux_in)
      JLM_UNREACHABLE("SHLS: loop_node::add_loopback_arg() cannot cast to node_input");

    auto mux = mux_in->node();

    auto mux_op = dynamic_cast<const jlm::static_hls::mux_op *>(&mux->operation());
    if (!mux_op)
      JLM_UNREACHABLE("SHLS: loop_node::add_loopback_arg() mux operation not found");
...
loop_node::is_op_implemented(const jlm::rvsdg::operation & op) const noexcept
{
  ...
  {
    if (static_cast<jlm::rvsdg::node_output *>(compute_subregion()->result(ind_res)->origin())
            ->node()
            ->operation()
        == op)
    {
      return static_cast<jlm::rvsdg::node_output *>(compute_subregion()->result(ind_res)->origin())
          ->node();
    }
  }
  return nullptr;
...
loop_node::get_origin_result(jlm::rvsdg::node_input * input) const
{
  ...
  if (!arg)
    return nullptr; // This is a loop_var
  return arg->result();
};
...
loop_node::print_nodes_registers() const
{
  ...
  {
    auto node_ouput =
        static_cast<jlm::rvsdg::node_output *>(compute_subregion()->result(i)->origin());

    auto node = node_ouput->node();
    std::cout << "node " << node->operation().debug_string() << " | ouput " << node_ouput->index()
              << " = ";

    auto backedge_result =
        dynamic_cast<jlm::static_hls::backedge_result *>(compute_subregion()->result(i));
    if (!backedge_result)
    {
    ...
    //   continue;
    // }
    // std::cout << "node ouput " << i << " : " << node_out->node()->operation().debug_string() <<
    // std::endl;
  }
};
...
loop_node::remove_single_input_muxes()
{
  ...
  {
    auto node_output =
        dynamic_cast<jlm::rvsdg::node_output *>(control_subregion()->result(i)->origin());
    if (!node_output)
      JLM_UNREACHABLE("SHLS: loop_node::remove_single_input_muxes() cannot cast to node_output");
...
loop_node::connect_muxes()
{
  for (size_t i = 0; i < control_subregion()->nresults(); i++)
  {
    auto node_output =
        dynamic_cast<jlm::rvsdg::node_output *>(control_subregion()->result(i)->origin());
    if (!node_output)
      JLM_UNREACHABLE("SHLS: loop_node::connect_muxes() cannot cast to node_output");
...
loop_node::finalize()
{
  ...
  auto arg = jlm::rvsdg::argument::create(
      control_subregion(),
      nullptr,
      jlm::rvsdg::ctltype::Create(fsm_->nalternatives()));
  connect_muxes();
  fsm_->apply_mux_ctl();
