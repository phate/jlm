...
private:
  mux_op(
      std::vector<std::shared_ptr<const jlm::rvsdg::type>> & operands_type,
      const std::shared_ptr<const jlm::rvsdg::type> & result_type)
...
public:
  ...
  create(jlm::rvsdg::output & predicate, const std::vector<jlm::rvsdg::output *> & alternatives)
  {
    ...
    auto operands_type = std::vector<std::shared_ptr<const jlm::rvsdg::type>>(
        alternatives.size() + 1,
...
public:
  ...
  reg_op(const std::shared_ptr<const jlm::rvsdg::type> & type)
      : jlm::rvsdg::simple_op(
          std::vector<std::shared_ptr<const jlm::rvsdg::type>>{ jlm::rvsdg::ctltype::Create(2),
                                                                type },
          { type })
  ...
  debug_string() const override
  {
    if (origin_debug_string_.empty())
      return jlm::util::strfmt("SHLS_REG", id_);
      ...
      std::string origin_debug_string)
  {
    ...
    // return jlm::rvsdg::simple_node::create_normalized(input.region(), op, { &store_input, &input
    // })[0];
  }
