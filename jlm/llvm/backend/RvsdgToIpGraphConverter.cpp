...
RvsdgToIpGraphConverter::ConvertSimpleNode(const rvsdg::SimpleNode & simpleNode)
{
  ...
  auto tac = ThreeAddressCode::create(
      std::unique_ptr<rvsdg::SimpleOperation>(
...
RvsdgToIpGraphConverter::ConvertPhiNode(const rvsdg::PhiNode & phiNode)
{
  ...
  for (size_t n = 0; n < subregion->nresults(); n++)
  {
    ...
    {
      auto op = util::assertedCast<const LlvmDeltaOperation>(&deltaNode->GetOperation());
...
void
RvsdgToIpGraphConverter::ConvertDeltaNode(const rvsdg::DeltaNode & deltaNode)
{
  auto & ipGraphModule = Context_->GetIpGraphModule();
