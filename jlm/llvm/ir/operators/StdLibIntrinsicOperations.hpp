 ...
 *
 * See [LLVM Language Reference
 * Manual](https://llvm.org/docs/LangRef.html#llvm-smax-intrinsic) for more details.
 */
class SMaxOperation final : public rvsdg::SimpleOperation
{
public:
  ~SMaxOperation() noexcept override;

  explicit SMaxOperation(const std::shared_ptr<const rvsdg::Type> & type)
      : SimpleOperation({ type, type }, { type })
  {
    checkType(type);
  }
  ...
  createTac(const Variable & operand1, const Variable & operand2)
  {
    auto operation = std::make_unique<SMaxOperation>(operand1.Type());
    return ThreeAddressCode::create(std::move(operation), { &operand1, &operand2 });
  }
  ...
  createNode(rvsdg::Output & operand1, rvsdg::Output & operand2)
  {
    return rvsdg::CreateOpNode<SMaxOperation>(
        {
            &operand1,
            &operand2,
        },
