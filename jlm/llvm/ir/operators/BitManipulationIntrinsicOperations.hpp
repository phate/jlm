 ...
 *
 * See [LLVM Language Reference
 * Manual](https://llvm.org/docs/LangRef.html#llvm-cttz-intrinsic) for more details.
 */
class CttzOperation final : public rvsdg::SimpleOperation
{
public:
  ~CttzOperation() noexcept override;

  explicit CttzOperation(const std::shared_ptr<const rvsdg::Type> & type)
      : SimpleOperation({ type, rvsdg::BitType::Create(1) }, { type })
  {
    checkType(type);
  }
  ...
  createTac(const Variable & operand, const Variable & isZeroPoison)
  {
    auto operation = std::make_unique<CttzOperation>(operand.Type());
    return ThreeAddressCode::create(std::move(operation), { &operand, &isZeroPoison });
  }
  ...
  createNode(rvsdg::Output & operand, rvsdg::Output & isZeroPoison)
  {
    return rvsdg::CreateOpNode<CttzOperation>({ &operand, &isZeroPoison }, operand.Type());
  }
