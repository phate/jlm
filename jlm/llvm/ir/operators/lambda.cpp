...
{
  argumentAttributes_.resize(operation().Type()->NumArguments());
}
...
node::GetFunctionArguments() const
{
  ...
  for (std::size_t n = 0; n < operation().Type()->Arguments().size(); ++n)
  {
...
node::MapInputContextVar(const rvsdg::input & input) const noexcept
{
  ...
  return ContextVar{ const_cast<rvsdg::input *>(&input),
                     subregion()->argument(operation().Type()->NumArguments() + input.index()) };
}
...
node::MapBinderContextVar(const rvsdg::output & output) const noexcept
{
  ...
  auto numArguments = operation().Type()->NumArguments();
  if (output.index() >= numArguments)
  {
    return ContextVar{ input(output.index() - operation().Type()->NumArguments()),
...
node::GetContextVars() const noexcept
{
  ...
  {
    vars.push_back(
        ContextVar{ input(n), subregion()->argument(n + operation().Type()->NumArguments()) });
  }
  return vars;
}
...
node::finalize(const std::vector<jlm::rvsdg::output *> & results)
{
  ...
  return append_output(std::make_unique<rvsdg::structural_output>(this, PointerType::Create()));
}
