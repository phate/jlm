    ...
    const std::vector<rvsdg::Output *> & operands)
{
  ...
  for (size_t n = 1; newLoadNode.noutputs(); n++)
  {
    auto & newMemoryStateOutput = *newLoadNode.output(n);
    JLM_ASSERT(is<MemoryStateType>(newMemoryStateOutput.Type()));
    auto & newMemoryStateInput = MapMemoryStateOutputToInput(newMemoryStateOutput);
    auto oldMemoryStateOperand = oldLoadMemoryStates[n - 1];

    if (newMemoryStateInput.origin() != oldMemoryStateOperand)
    {
      const auto memoryStateMergeNode =
          rvsdg::TryGetOwnerNode<rvsdg::SimpleNode>(*oldMemoryStateOperand);
      JLM_ASSERT(is<MemoryStateMergeOperation>(memoryStateMergeNode));

      auto memoryStateMergeOperands = rvsdg::Outputs(
          newLoadNode,
          newMemoryStateOutput.index(),
          memoryStateMergeNode->ninputs());
      const auto result = MemoryStateMergeOperation::CreateNode(memoryStateMergeOperands).output(0);
      results.push_back(result);
    }
    else
    {
      results.push_back(&newMemoryStateOutput);
    }
  }
