...
static const bool ONLY_LIVE_ALLOCAS = std::getenv("ONLY_LIVE_ALLOCAS");
static const bool SKIP_NON_REENTRANT_ALLOCAS = std::getenv("SKIP_NON_REENTRANT_ALLOCAS");
...
public:
  ...
  /**
   * Creates a new ModRefSet that is not mapped to anything
  ...
  [[nodiscard]] bool
  HasSetForRegion(const rvsdg::Region & region) const
  {
    return RegionMap_.find(&region) != RegionMap_.end();
  }

  [[nodiscard]] ModRefSetIndex
  GetSetForRegion(const rvsdg::Region & region) const
  {
    const auto it = RegionMap_.find(&region);
    JLM_ASSERT(it != RegionMap_.end());
    return it->second;
  }

  [[nodiscard]] ModRefSetIndex
  GetOrCreateSetForRegion(const rvsdg::Region & region)
  {
  ...
  // The rest of this class is the implementation of virtual functions
  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForRegion(const rvsdg::Region & region) const override
  {
    return ModRefSets_[GetSetForRegion(region)].GetMemoryNodes();
  }

  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForNode(const rvsdg::Node & node) const override
  {
    return ModRefSets_[GetSetForNode(node)].GetMemoryNodes();
  }

  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForLambda(const rvsdg::LambdaNode & lambdaNode) const override
  {
    return ModRefSets_[GetSetForNode(lambdaNode)].GetMemoryNodes();
  }

  /*
  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForLoad(const rvsdg::SimpleNode & loadNode) const override
  {
    JLM_ASSERT(is<LoadOperation>(&loadNode));
    return ModRefSets_[GetSetForNode(loadNode)].GetMemoryNodes();
  }

  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForStore(const rvsdg::SimpleNode & storeNode) const override
  {
    JLM_ASSERT(is<StoreOperation>(&storeNode));
    return ModRefSets_[GetSetForNode(storeNode)].GetMemoryNodes();
  }

  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForMemcpy(const rvsdg::SimpleNode & memcpyNode) const override
  {
    JLM_ASSERT(is<MemCpyOperation>(&memcpyNode));
    return ModRefSets_[GetSetForNode(memcpyNode)].GetMemoryNodes();
  }

  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForFree(const rvsdg::SimpleNode & freeNode) const override
  {
    JLM_ASSERT(is<FreeOperation>(&freeNode));
    return ModRefSets_[GetSetForNode(freeNode)].GetMemoryNodes();
  }

  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForCall(const rvsdg::SimpleNode & callNode) const override
  {
    JLM_ASSERT(is<CallOperation>(&callNode));
    return ModRefSets_[GetSetForNode(callNode)].GetMemoryNodes();
  }

  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForTheta(const rvsdg::ThetaNode & thetaNode) const override
  {
    return ModRefSets_[GetSetForNode(thetaNode)].GetMemoryNodes();
  }

  const util::HashSet<const PointsToGraph::MemoryNode *> &
  GetModRefForGamma(const rvsdg::GammaNode & gammaNode) const override
  {
    return ModRefSets_[GetSetForNode(gammaNode)].GetMemoryNodes();
  }
...
private:
  ...
  /**
   * Map from regions to their ModRefSet.
   */
  std::unordered_map<const rvsdg::Region *, ModRefSetIndex> RegionMap_;
  /**
   * Map from nodes that have memory side effects, to their ModRefSet.
   * Includes nodes like loads, stores, memcpy, free and calls.
   * Also includes structural nodes, i.e., gamma and theta.
   */
...
struct RegionAwareModRefSummarizer::Context
{
  ...
  /**
   * For each SCC, the set of Alloca MemoryNodes that may be live when a function from the SCC
   * is at the top of the call stack.
   * @see FindAllocasLiveInSccs()
   */
  std::vector<util::HashSet<const PointsToGraph::MemoryNode *>> AllocasLiveInScc;
