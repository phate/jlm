...
public:
   ...
   *
   * In the PointerObjectSet, the PointsToExternal flag encodes pointing to an address from outside the module.
   * This may however be the address of a memory object within the module, that has escaped.
   * In the final graph, any node marked as pointing to external, will get an edge to the special "external" node,
   * as well as to every memory object node marked as escaped.
   *
