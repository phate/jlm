...
Andersen::AnalyzeRvsdg(const rvsdg::Graph & graph)
{
  ...
  {
    auto & argument = *util::assertedCast<LlvmGraphImport>(rootRegion.argument(n));
    ...
    Statistics & statistics)
{
  ...
  for (auto [allocaNode, pointerObjectIndex] : set.GetAllocaMap())
  {
    memoryNodeMapping[pointerObjectIndex] =
        pointsToGraph->addNodeForAlloca(*allocaNode, set.HasEscaped(pointerObjectIndex));
  }

  for (auto [deltaNode, pointerObjectIndex] : set.GetGlobalMap())
  {
    memoryNodeMapping[pointerObjectIndex] =
        pointsToGraph->addNodeForDelta(*deltaNode, set.HasEscaped(pointerObjectIndex));
  }

  for (auto [import, pointerObjectIndex] : set.GetImportMap())
  {
    memoryNodeMapping[pointerObjectIndex] =
        pointsToGraph->addNodeForImport(*import, set.HasEscaped(pointerObjectIndex));
  }

  for (auto [lambdaNode, pointerObjectIndex] : set.GetFunctionMap())
  {
    memoryNodeMapping[pointerObjectIndex] =
        pointsToGraph->addNodeForLambda(*lambdaNode, set.HasEscaped(pointerObjectIndex));
  }

  for (auto [mallocNode, pointerObjectIndex] : set.GetMallocMap())
  {
    memoryNodeMapping[pointerObjectIndex] =
        pointsToGraph->addNodeForMalloc(*mallocNode, set.HasEscaped(pointerObjectIndex));
  }
  ...
  {
    const auto ptgNode = pointsToGraph->addNodeForRegisters();
