...
public:
   ...
   *
   * A recursively variable from the POV of a phi node has
   * multiple representations. This structure bundles
   * all representations into a single object.
   */
  struct FixVar
  {
    /**
     * \brief Reference to mutually recursively defined object in phi.
     *
   ...
   *   and the must be identity mappings within the phi region.
   *
  ...
  /**
   * \brief Gets all all fixpoint variables.
   *
   * \returns
