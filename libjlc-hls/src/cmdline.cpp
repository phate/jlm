...
parse_cmdline(int argc, char ** argv, jlm::cmdline_options & options)
{
	...
	*/
	...
	cl::list<std::string> hls_functiom(
	  "hls-function"
	, cl::Prefix
	...
	if (ifiles.empty()) {
		std::cerr << "jlc-hls: no input files.\n";
		exit(EXIT_FAILURE);
	}

	if (ifiles.size() > 1 && no_linking && !ofilepath.empty()) {
		std::cerr << "jlc-hls: cannot specify -o when generating multiple output files.\n";
		exit(EXIT_FAILURE);
	}

	if (hls_functiom.empty()) {
		std::cerr << "jlc-hls: no function regex specified\n";
		exit(EXIT_FAILURE);
	}

	if (hls_functiom.size() > 1) {
	...
	options.hls_function_regex = hls_functiom.front();
